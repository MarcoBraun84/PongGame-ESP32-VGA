...
Über ein Piezo werden Spielgeräusche, wie Ballaufprallgeräusche und Punktemelodie, erzeugt.
...
Die Bildübertragung benötigt 6 Adern (R, G, B, V-Syc, H-Syc, GND) des VGA-Kabels, um ein Bild mit 3-Bit-Farbraum zu übertragen. Diese können durch Jumper-Kabel direkt vom VGA-Kabelstecker abgegriffen werden (siehe Bild).

Alternativ kann das VGA-Kabel direkt verlötet werden. Hierbei bietet es sich an, die einzelnen Adern mit einer Durchgangsprüfung zu bestimmen.
...
Auf der Vorderseite befindet sich mittig ein "PONG" Schriftzug und zwei kleinere Spielerbezeichnungen, die für die passende Zuordnung der Kontroller und der darüber liegenden Funktionsknöpfe dienen.
...
Die Kontroller wurden so designt, dass der MPU6050 in das Gehäuse gesteckt werden kann und selbstständig hält. Auf der Oberseite befindet sich die Spielerbezeichnung P1 bzw. P2.
Die Kontroller-Form ermöglicht dem Spieler, den Kontroller einhändig auf dem Tisch oder zweihändig zu bedienen. 
...
### MPU6050 Steuerung
Leider unterstützt die genutzte Library [MPU6050_tockn](https://github.com/Tockn/MPU6050_tockn) keine Adressierung des MPU6050, wodurch mehrere MPUs nicht standardmäßig möglich sind. Lediglich durch Dublizieren dieser Library und Ändern der I2C-Adresse werden damit zwei MPUs ermöglicht (AD0 mit 3,3 V ändert die Adresse zu x069 mit [MPU605069-Library](https://github.com/tiimiiiiiiiiii/pong-esp32-vga/blob/main/MPU6050_tockn69.zip)).

Um die Funktion der beiden MPUs zu testen, kann der folgende Code verwendet werden. Hierbei werden über den seriellen Monitor die Werte der X-Achsen-Auslenkung ausgegeben.
...
```
Anmerkung: Beim Start des Codes wird eine automatische Kalibrierung der MPUs vorgenommen.
